# streamlit_app.py  (duplicate widget IDs fixed)
...
def format_text(txt):
    try:
        return _fmt_uncached(_clean(txt))
    except Exception as e:
        st.error(f"Format-text error → {e}")
        return _clean(txt)
...
def _manu_uncached(txt):
    res = client.chat.complete(
        ...
        messages=[{
            "role": "user",
            "content": ("Extract manufacturer / maker names separated by hyphen "
                        "— plain list only\ncontent: " + txt),
        }],
    )
...
def manufacture_name(txt):
    try:
        return _manu_uncached(_clean(txt))
    except Exception as e:
        st.error(f"Manufacturer-name error → {e}")
        return ""
...
with col1:
    ...
    with st.expander("Excel templates", True):
        upl_tpl = st.file_uploader("Upload template", type=["xlsx"], key="tpl_upload")
        fin_tpl = st.file_uploader("Final Sheet template", type=["xlsx"], key="tpl_final")

    upl_tpl = upl_tpl or "upload file - HTS.xlsx"
    fin_tpl = fin_tpl or "FINAL SHEET.xlsx"

    if techno:
        suffix = st.text_input("Output name suffix (no ext)", key="suffix_excel")
        save_to = st.text_input("Folder to save files", key="folder_excel")
        if st.button("🚀 Process Excel", key="proc_excel") and suffix and save_to:
            try:
                ...
                sheet_ok = next(
                    (s for s in xls.sheet_names
                     if req.issubset(set(pd.read_excel(xls, s, nrows=1).columns))),
                    None,
                )
                if not sheet_ok:
...
with col2:
    ...
    with st.expander("Excel templates", True):
        ...
        fin_tpl_p = st.file_uploader("Final Sheet template", type=["xlsx"], key="tpl_final_pdf")
    ...
    if pdf:
        ...
        save_to = st.text_input("Folder to save files", key="folder_pdf")
        if st.button("🚀 Process PDF", key="proc_pdf") and hts_no and save_to:
            try:
...
with col3:
    ...
    hts_up = st.file_uploader("upload file – HTS.xlsx", type=["xlsx"], key="hts_clean")
